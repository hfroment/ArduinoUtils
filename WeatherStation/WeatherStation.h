...
public:
    ...
    WeatherStation(bool useRtc = false, bool temperaturePresent = false, uint8_t lcdAddress = 0, eTypeLcd typeLcd = LCD_1602, uint8_t humidityType = 0 /*absent*/, bool pressurePresent = false, bool useLcd = true);
    ...
    bool getPressure(float &stationPressure, float& seaPressure, int &forecast);

    void setTime(uint32_t epoch);
    bool getTime(DateTime& datetime);

    void log(uint8_t line, String& chaine);
    void clearScreen();
    void setBacklight(bool on);

    const String& forecastString(int forecast);
